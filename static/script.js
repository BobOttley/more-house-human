...
document.addEventListener("DOMContentLoaded", () => {
  ...
  socket.on("response", (data) => {
      removeThinking();
      const html = renderParagraphs(data.message.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank">$1</a>'));
  ...
  const quickByCat = {
      ...
      lunch: [{ label: "Lunch menu", query: "Where can I find the school lunch menu?" }],
      calendar: [
          { label: "Term dates", query: "What are the term dates?" },
          { label: "Open events", query: "What are the open events?" }
      ],
      uniform: [{ label: "Uniform", query: "What is the school uniform?" }],
      scholarships: [{ label: "Bursaries & scholarships", query: "Tell me about scholarships and bursaries" }],
      contact: [{ label: "Contact us", query: "How can I contact the school?" }],
      academics: [
  ...
  function renderBot(html, source, isWelcome = false, category = "admissions", save = true) {
      ...
      if (source === "bot") {
          ...
          wrp.innerHTML = finalBtns.map(b => `<button class="quick-reply" data-query="${b.query}">${b.label}</button>`).join("");
          d.appendChild(wrp);
      }
      msgs.scrollTop = msgs.scrollHeight;
